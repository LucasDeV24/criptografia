...
const code2: CodeChallenge = {
  ...
  instructions: 'Simule um ataque rainbow table: construa um dicionário hash→senha com senhas comuns, depois tente quebrar o hash de "admin".',
  languages: ['javascript', 'python'],
  starterCode: {
    javascript: `function hashSimples(texto) {\n  let hash = 0;\n  for (let i = 0; i < texto.length; i++) {\n    hash = ((hash << 5) - hash) + texto.charCodeAt(i);\n    hash = hash & hash;\n  }\n  return Math.abs(hash).toString(16);\n}\n\nconst senhasComuns = ["123456", "password", "admin", "qwerty", "letmein"];\n\n// 1. Crie um objeto rainbowTable (vazio {})\n// 2. Para cada senha comum, calcule o hash e adicione:\n//    rainbowTable[hash] = senha\n// 3. Calcule o hash de "admin" (hashRoubado)\n// 4. Busque na rainbow table:\n//    Se encontrou: "Senha quebrada: " + senha\n//    Senao: "Senha nao encontrada na rainbow table"\n`,
    python: `def hash_simples(texto):\n    h = 0\n    for c in texto:\n        h = ((h * 31) + ord(c)) & 0xFFFFFFFF\n    return hex(h)[2:]\n\nsenhas_comuns = ["123456", "password", "admin", "qwerty", "letmein"]\n\n# 1. Crie um dicionario rainbow_table (vazio {})\n# 2. Para cada senha comum, calcule o hash e adicione:\n#    rainbow_table[hash] = senha\n# 3. Calcule o hash de "admin" (hash_roubado)\n# 4. Busque na rainbow table:\n#    Se encontrou: "Senha quebrada: " + senha\n#    Senao: "Senha nao encontrada na rainbow table"\n`,
  },
