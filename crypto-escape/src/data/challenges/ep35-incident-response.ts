...
const code1: CodeChallenge = {
  ...
  instructions: 'Execute e veja a classificação.',
  languages: ['javascript', 'python'],
  starterCode: {
    javascript: `function classificar(incidente) {\n  if (incidente.dadosVazados || incidente.sistemaComprometido) {\n    return "CRITICO";\n  }\n  if (incidente.ataqueAtivo) {\n    return "ALTO";\n  }\n  if (incidente.tentativaDetectada) {\n    return "MEDIO";\n  }\n  return "BAIXO";\n}\n\nconsole.log(classificar({ dadosVazados: true, ataqueAtivo: true }));\nconsole.log(classificar({ ataqueAtivo: true }));\nconsole.log(classificar({ tentativaDetectada: true }));\nconsole.log(classificar({ scanPortas: true }));\n`,
    python: `def classificar(incidente):\n    if incidente.get("dadosVazados") or incidente.get("sistemaComprometido"):\n        return "CRITICO"\n    if incidente.get("ataqueAtivo"):\n        return "ALTO"\n    if incidente.get("tentativaDetectada"):\n        return "MEDIO"\n    return "BAIXO"\n\nprint(classificar({"dadosVazados": True, "ataqueAtivo": True}))\nprint(classificar({"ataqueAtivo": True}))\nprint(classificar({"tentativaDetectada": True}))\nprint(classificar({"scanPortas": True}))\n`,
  },
