...
const code1: CodeChallenge = {
  ...
  instructions: 'Execute e veja o firewall funcionando.',
  languages: ['javascript', 'python'],
  starterCode: {
    javascript: `const regras = [\n  { acao: "PERMITIR", porta: 443, descricao: "HTTPS" },\n  { acao: "PERMITIR", porta: 80, descricao: "HTTP" },\n  { acao: "BLOQUEAR", porta: 22, descricao: "SSH" },\n  { acao: "BLOQUEAR", porta: 23, descricao: "Telnet" }\n];\n\nfunction avaliar(porta) {\n  for (let i = 0; i < regras.length; i++) {\n    if (regras[i].porta == porta) {\n      return regras[i].acao + ": porta " + porta + " (" + regras[i].descricao + ")";\n    }\n  }\n  return "BLOQUEAR: porta " + porta + " (padrao)";\n}\n\nconsole.log(avaliar(443));\nconsole.log(avaliar(22));\nconsole.log(avaliar(3306));\n`,
    python: `regras = [\n    {"acao": "PERMITIR", "porta": 443, "descricao": "HTTPS"},\n    {"acao": "PERMITIR", "porta": 80, "descricao": "HTTP"},\n    {"acao": "BLOQUEAR", "porta": 22, "descricao": "SSH"},\n    {"acao": "BLOQUEAR", "porta": 23, "descricao": "Telnet"}\n]\n\ndef avaliar(porta):\n    for regra in regras:\n        if regra["porta"] == porta:\n            return regra["acao"] + ": porta " + str(porta) + " (" + regra["descricao"] + ")"\n    return "BLOQUEAR: porta " + str(porta) + " (padrao)"\n\nprint(avaliar(443))\nprint(avaliar(22))\nprint(avaliar(3306))\n`,
  },
...
const code2: CodeChallenge = {
  ...
  instructions: 'Execute e veja o filtro por IP.',
  languages: ['javascript', 'python'],
  starterCode: {
    javascript: `const blacklist = ["10.0.0.99", "45.33.32.1", "192.168.1.666"];\n\nfunction verificarIP(ip) {\n  for (let i = 0; i < blacklist.length; i++) {\n    if (ip == blacklist[i]) {\n      return "BLOQUEADO: " + ip + " esta na blacklist";\n    }\n  }\n  return "PERMITIDO: " + ip;\n}\n\nconsole.log(verificarIP("192.168.1.1"));\nconsole.log(verificarIP("45.33.32.1"));\nconsole.log(verificarIP("8.8.8.8"));\n`,
    python: `blacklist = ["10.0.0.99", "45.33.32.1", "192.168.1.666"]\n\ndef verificar_ip(ip):\n    for bl in blacklist:\n        if ip == bl:\n            return "BLOQUEADO: " + ip + " esta na blacklist"\n    return "PERMITIDO: " + ip\n\nprint(verificar_ip("192.168.1.1"))\nprint(verificar_ip("45.33.32.1"))\nprint(verificar_ip("8.8.8.8"))\n`,
  },
