...
const code1: CodeChallenge = {
  ...
  instructions: 'Verifique quais headers de segurança recomendados estão presentes no servidor. Imprima "OK" ou "FALTANDO" para cada.',
  languages: ['javascript', 'python'],
  starterCode: {
    javascript: `const headers = {\n  "Content-Type": "text/html",\n  "X-Frame-Options": "DENY",\n  "Access-Control-Allow-Origin": "*"\n};\n\nconst headersRecomendados = [\n  "Content-Security-Policy",\n  "X-Frame-Options",\n  "X-Content-Type-Options",\n  "Strict-Transport-Security"\n];\n\n// Percorra headersRecomendados e para cada:\n// - Se existe em headers: console.log("OK: " + h)\n// - Senao: console.log("FALTANDO: " + h)\n`,
    python: `headers = {\n    "Content-Type": "text/html",\n    "X-Frame-Options": "DENY",\n    "Access-Control-Allow-Origin": "*"\n}\n\nheaders_recomendados = [\n    "Content-Security-Policy",\n    "X-Frame-Options",\n    "X-Content-Type-Options",\n    "Strict-Transport-Security"\n]\n\n# Percorra headers_recomendados e para cada:\n# - Se existe em headers: print("OK: " + h)\n# - Senao: print("FALTANDO: " + h)\n`,
  },
...
const code2: CodeChallenge = {
  ...
  instructions: 'Crie uma função que analisa o header CORS e retorna se é perigoso (*), seguro (domínio específico) ou ausente.',
  languages: ['javascript', 'python'],
  starterCode: {
    javascript: `// Crie verificarCORS(headers):\n// - Pegue o valor de "Access-Control-Allow-Origin"\n// - Se nao existe: retorne "SEM CORS: API nao acessivel externamente"\n// - Se == "*": retorne "PERIGO: qualquer site pode acessar sua API!"\n// - Senao: retorne "SEGURO: acesso restrito a " + valor\n\n\nconsole.log(verificarCORS({"Access-Control-Allow-Origin": "*"}));\nconsole.log(verificarCORS({"Access-Control-Allow-Origin": "https://meusite.com"}));\nconsole.log(verificarCORS({}));\n`,
    python: `# Crie verificar_cors(headers):\n# - Use headers.get("Access-Control-Allow-Origin", "") para pegar o valor\n# - Se vazio: retorne "SEM CORS: API nao acessivel externamente"\n# - Se == "*": retorne "PERIGO: qualquer site pode acessar sua API!"\n# - Senao: retorne "SEGURO: acesso restrito a " + valor\n\n\nprint(verificar_cors({"Access-Control-Allow-Origin": "*"}))\nprint(verificar_cors({"Access-Control-Allow-Origin": "https://meusite.com"}))\nprint(verificar_cors({}))\n`,
  },
...
const code3: CodeChallenge = {
  ...
  instructions: 'Calcule a pontuação de segurança do servidor verificando quais headers estão presentes, cada um com um peso diferente.',
  languages: ['javascript', 'python'],
  starterCode: {
    javascript: `const servidor = {\n  "Content-Security-Policy": "default-src 'self'",\n  "X-Frame-Options": "DENY",\n  "X-Content-Type-Options": "nosniff",\n  "Access-Control-Allow-Origin": "https://app.com"\n};\n\nconst regras = [\n  { header: "Content-Security-Policy", peso: 3 },\n  { header: "X-Frame-Options", peso: 2 },\n  { header: "X-Content-Type-Options", peso: 1 },\n  { header: "Strict-Transport-Security", peso: 3 },\n  { header: "Access-Control-Allow-Origin", peso: 2 }\n];\n\n// Calcule a pontuacao de seguranca:\n// - Percorra regras, some os pesos no total\n// - Se o header existe em servidor, some ao pontos\n// - Imprima: "Seguranca: " + pontos + "/" + total + " pontos"\n`,
    python: `servidor = {\n    "Content-Security-Policy": "default-src 'self'",\n    "X-Frame-Options": "DENY",\n    "X-Content-Type-Options": "nosniff",\n    "Access-Control-Allow-Origin": "https://app.com"\n}\n\nregras = [\n    {"header": "Content-Security-Policy", "peso": 3},\n    {"header": "X-Frame-Options", "peso": 2},\n    {"header": "X-Content-Type-Options", "peso": 1},\n    {"header": "Strict-Transport-Security", "peso": 3},\n    {"header": "Access-Control-Allow-Origin", "peso": 2}\n]\n\n# Calcule a pontuacao de seguranca:\n# - Percorra regras, some os pesos no total\n# - Se o header existe em servidor, some ao pontos\n# - Imprima: "Seguranca: " + str(pontos) + "/" + str(total) + " pontos"\n`,
  },
